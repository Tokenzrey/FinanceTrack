...
- **Tugas** (web + bot): kelola daftar tugas dengan prioritas (tinggi/sedang/rendah), batas waktu (due date), dan status (belum/sedang/selesai). Web punya **tiga proyeksi dari data yang sama**: daftar, **Papan (Kanban)** dengan drag-and-drop antar kolom, dan **Linimasa (Gantt)** dengan bar jadwal (start→due) plus pin pengingat. Akses cepat via bot (`/tugas`, `/selesai <no>`); kartu tugas di chat menyebut kolom papan dan label bila ada. Bot mengingatkan tugas jatuh tempo langsung di chat.
- **Detail tugas** (web): modal detail berisi checklist, dependensi antar-tugas (dengan deteksi siklus), catatan progres, dan lampiran tautan.
...
- **Firestore collections**: `users/{uid}/tasks`, `users/{uid}/notes`, `users/{uid}/reminders`, `users/{uid}/lists` (kolom papan), `users/{uid}/labels` (label berwarna) (per pengguna); preferensi di `users/{uid}/meta/plannerPrefs`; roster pengguna opt-in digest di `bot_meta/digestRoster` (root, hanya Admin SDK). `status` (dipakai bot) dan `listId` (kolom papan) **selalu sinkron** — memindahkan kartu di papan menulis `status`, dan `/selesai` dari bot ikut memindahkan kartunya; kolom mana yang memetakan ke status mana disimpan di `lists.{listId}.mapsToStatus`.
